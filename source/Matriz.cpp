...
Vetor Matriz::operator*(Vetor outroVetor) {
    assert(mColunas == outroVetor.GetNumElementos());
    double soma;
    Vetor armVetor(outroVetor);
    Vetor saida(outroVetor.GetNumElementos());
    for (int i = 0; i < mLinhas; i++)
    {
        soma = 0;
        for (int j = 0; j < mColunas; j++)
        {
            soma += mData[i][j] * armVetor[j]; 
        }
        saida[i] = soma;
    }
    return saida;
